...
[v1.2.0](https://github.com/chipkin/BACnetProfileExample-B-AAC-CPP/releases/tag/v1.2.0)
(`metrics-windows.json` / `metrics-linux.json`):

<!-- METRICS -->
| Platform | Binary | Size | Start-up to `ready` | Stack commit |
|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBAAC.exe` | 3,408,896 bytes (~3.3 MiB) | 74 ms | `abd4cee1` |
| Linux x64 (ubuntu-latest) | `BACnetExampleBAAC` | 49,720 bytes (~49 KiB) | 108 ms | `abd4cee1` |

v1.2.0 was published from a prebuilt stack library rather than the build documented
above; the next release refreshes these numbers from the documented build.
