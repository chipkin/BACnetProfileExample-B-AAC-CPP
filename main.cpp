...
// with a low/high limit. When its Present_Value crosses a limit, the stack fires a
// ConfirmedEventNotification to the recipients of Notification Class 1 ("Jade").
// Drive Diamond out of range with a WriteProperty to its Present_Value to see it.
//
...
bool GetPropertyEnumerated(const uint32_t deviceInstance, const uint16_t objectType,
    ...
    // Analog Value 1 "Diamond" Units (no-units here; report the generic units enum).
    if (objectType == OBJECT_TYPE_ANALOG_VALUE &&
        objectInstance == ANALOG_VALUE_INSTANCE &&
        propertyIdentifier == PROPERTY_IDENTIFIER_UNITS) {
        *value = ENGINEERING_UNITS_DEGREES_CELSIUS; // reuse the one unit this example defines
        return true;
    }
...
// The compare is length-checked first (so it never reads past the wire buffer,
// which is NOT null-terminated) and folds the byte comparison into one accumulator
// so it does not short-circuit on the first wrong byte - a constant-time-style
// compare that avoids leaking how much of the password matched via timing.
static bool PasswordAccepted(const char* password, uint8_t passwordLength) {
...
bool ReinitializeDevice(const uint32_t deviceInstance, const uint32_t reinitializedState,
    ...
    if (!PasswordAccepted(password, (uint8_t)passwordLength)) {
    ...
    // Backup/restore states (2..6) are not supported by this example.
...
// application react (and could reject with an error). The callback carries a long
// argument list (the acknowledged event, its time stamp, the ack source, and the
// time of acknowledgement); this example only needs the object that was acked.
bool AcknowledgeAlarm(const uint32_t deviceInstance, const uint32_t acknowledgingProcessIdentifier,
    ...
    (void)acknowledgingProcessIdentifier;
    (void)eventStateAcknowledged;
    (void)eventTimeStampYear; (void)eventTimeStampMonth; (void)eventTimeStampDay;
    (void)eventTimeStampWeekday; (void)eventTimeStampHour; (void)eventTimeStampMinute;
    (void)eventTimeStampSecond; (void)eventTimeStampHundrethSecond;
    (void)acknowledgementSource; (void)acknowledgementSourceLength; (void)acknowledgementSourceEncoding;
    (void)timeOfAcknowledgementIsTime; (void)timeOfAcknowledgementIsSequenceNumber;
    (void)timeOfAcknowledgementIsDateTime; (void)timeOfAcknowledgementYear;
    (void)timeOfAcknowledgementMonth; (void)timeOfAcknowledgementDay; (void)timeOfAcknowledgementWeekday;
    (void)timeOfAcknowledgementHour; (void)timeOfAcknowledgementMinute; (void)timeOfAcknowledgementSecond;
    (void)timeOfAcknowledgementHundrethSecond; (void)timeOfAcknowledgementSequenceNumber;
    (void)errorCode;
    if (deviceInstance != g_deviceInstance) {
        return false;
    }
    printf("AcknowledgeAlarm: object (type %u, instance %u) acknowledged\n",
...
int main(int argc, char** argv) {
    ...
    uint8_t recipientIp[4];
    if (RECIPIENT_USE_BROADCAST) {
        recipientIp[0] = (uint8_t)(g_ipAddress[0] | ~g_ipSubnetMask[0]);
        recipientIp[1] = (uint8_t)(g_ipAddress[1] | ~g_ipSubnetMask[1]);
        recipientIp[2] = (uint8_t)(g_ipAddress[2] | ~g_ipSubnetMask[2]);
        recipientIp[3] = (uint8_t)(g_ipAddress[3] | ~g_ipSubnetMask[3]);
    } else {
        memcpy(recipientIp, RECIPIENT_IP, 4);
    }
    const uint8_t recipientMac[6] = {
        recipientIp[0], recipientIp[1], recipientIp[2], recipientIp[3],
        (uint8_t)(g_bacnetIpUdpPort >> 8), (uint8_t)(g_bacnetIpUdpPort & 0xFF)
    };
